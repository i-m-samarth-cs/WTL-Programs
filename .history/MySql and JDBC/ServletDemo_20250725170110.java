...
public class ServletDemo extends HttpServlet {
    ...
    private void handleRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // FIX 1: Change deprecated driver class name to the new one (com.mysql.cj.jdbc.Driver)
            // Even though SPI auto-registers, explicitly loading it can help avoid issues in some setups
            Class.forName("com.mysql.cj.jdbc.Driver");

            // FIX 2: Update connection URL for modern MySQL Connector/J
            // Adding useSSL=false and allowPublicKeyRetrieval=true for development/local setup
            ...
            // Statement stmt is created but not used for a query, which is fine for just testing connection.
            ...
            e.printStackTrace();
            // Also print to the browser for immediate feedback during development (remove in production)
            if (out != null) {
            ...
            if (out != null) {
                ...
                out.print("<p>Details: " + e.getMessage() + "</p>");
                out.print("<p>Make sure 'mysql-connector-j-9.4.0.jar' (or your version) is in Tomcat's 'lib' folder.</p>");
                out.print("<pre>");
                e.printStackTrace(out); // Print stack trace to browser for debugging
                out.print("</pre>");
            }
            throw new ServletException("JDBC Driver not found. Make sure mysql-connector-j-x.x.x.jar is in Tomcat's lib folder.", e);

        } finally {
            // FIX 3: Close the Connection in a finally block to ensure it's closed even if errors occur.
