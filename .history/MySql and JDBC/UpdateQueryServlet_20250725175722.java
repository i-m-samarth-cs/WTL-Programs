...
public class UpdateQueryServlet extends HttpServlet {
    public void service(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<html>");
        out.println("<head><title>Update Status</title></head>");
        out.println("<body>");
        ...
        try {
            ...
            if (studRollStr != null && studRollStr.length() > 0 && studAddress != null && studAddress.length() > 0) {
                try {
                    int rollNo = Integer.parseInt(studRollStr);
                    String sql = "UPDATE students_table SET address = '" + studAddress + "' WHERE Roll_no = " + rollNo;
                    int rowsAffected = stmt.executeUpdate(sql);
                    if (rowsAffected > 0) {
                        out.println("<h2>Record with Roll No. " + rollNo + " Updated Successfully!</h2>");
                    } else {
                        out.println("<h2>No record found with Roll No. " + rollNo + " to update.</h2>");
                    }
                } catch (NumberFormatException e) {
                    out.println("<h2>Invalid Roll Number. Please enter a numeric value.</h2>");
                }
            } else {
                out.println("<h2>Please enter both Roll Number and New Address to update.</h2>");
            }
